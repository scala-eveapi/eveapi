...
case class Eveapi(currentTime: String,
                  result: eveapi.xml.account.char.WalletTransactions.Result,
                  cachedUntil: String,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
  lazy val version = attributes("@version").as[BigInt]
}

case class Result(rowset: eveapi.xml.account.char.WalletTransactions.Rowset)

case class Rowset(row: Seq[eveapi.xml.account.char.WalletTransactions.Row] = Nil,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
