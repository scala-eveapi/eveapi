...
case class Eveapi(currentTime: String,
                  result: eveapi.xml.account.account.Characters.Result,
                  cachedUntil: String,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
  lazy val version = attributes("@version").as[BigInt]
}

case class Result(rowset: eveapi.xml.account.account.Characters.Rowset)

case class Rowset(row: Seq[eveapi.xml.account.account.Characters.Row] = Nil,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
