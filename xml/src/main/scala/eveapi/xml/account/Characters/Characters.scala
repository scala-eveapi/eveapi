...
case class Eveapi(currentTime: String,
                  result: eveapi.xml.account.Characters.Result,
                  cachedUntil: String,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
  lazy val version = attributes("@version").as[BigInt]
}

case class Result(rowset: eveapi.xml.account.Characters.Rowset)

case class Rowset(row: Seq[eveapi.xml.account.Characters.Row] = Nil,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
