...
case class Eveapi(currentTime: String,
                  result: eveapi.xml.account.account.APIKeyInfo.Result,
                  cachedUntil: String,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
  lazy val version = attributes("@version").as[BigInt]
}

case class Result(key: eveapi.xml.account.account.APIKeyInfo.Key)

case class Key(rowset: eveapi.xml.account.account.APIKeyInfo.Rowset,
               attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
...
case class Rowset(row: Seq[eveapi.xml.account.account.APIKeyInfo.Row] = Nil,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
