...
case class Eveapi(currentTime: String,
                  result: eveapi.xml.map.Sovereignty.Result,
                  cachedUntil: String,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
  lazy val version = attributes("@version").as[BigInt]
}

case class Result(rowset: eveapi.xml.map.Sovereignty.Rowset, dataTime: String)

case class Rowset(row: Seq[eveapi.xml.map.Sovereignty.Row] = Nil,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
