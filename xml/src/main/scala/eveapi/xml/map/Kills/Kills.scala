...
case class Eveapi(currentTime: String,
                  result: eveapi.xml.map.Kills.Result,
                  cachedUntil: String,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
  lazy val version = attributes("@version").as[BigInt]
}

case class Result(rowset: eveapi.xml.map.Kills.Rowset, dataTime: String)

case class Rowset(row: Seq[eveapi.xml.map.Kills.Row] = Nil,
                  attributes: Map[String, scalaxb.DataRecord[Any]] = Map()) {
